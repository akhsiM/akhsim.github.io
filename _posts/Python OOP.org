...
Calling a method is different from calling an attribute. It requires a pair of ~()~ at the end of the method.
...
#+Name: Without the Dunder repr method:
#+BEGIN_SRC python :exports both :results output :session
class Employee:
	pay_raise_amt = 1.04
...
#+Name: With the Dunder repr method:
#+BEGIN_SRC python :exports both :results output :session
class Employee:
	pay_raise_amt = 1.04
...
#+RESULTS: With the Dunder repr method:
...
#+RESULTS: With the Dunder repr and str method:
...
~str()~ returns a human readable string whereas ~repr()~ returns a python friendly formated string. ~repr()~'s goal is to be unambiguous whereas ~str()~'s goal is to be human readable.
(More on this topic: https://stackoverflow.com/questions/1436703/difference-between-str-and-repr)
...
#+Name: Use + for combining salaries
#+BEGIN_SRC python :exports both :results output :session
class Employee:
	pay_raise_amt = 1.04
...
#+RESULTS: Use + for combining salaries
: 105000
...
It depends on the nature and use case of the variable of the class and the class variables. However there are scenarios where this overriding is a must-have. For example, ~Employee.number_of_employees~ would be a Class Variable where there would be no difference between individual instance. 
...
See more about ~super()~ [[super][here]].
...
A common use case for this is building classes that access/ extends to functionality of a previously built class (inheritance). This saves us the time and effort from needing to rewrite previously defined methods and attributes in subclasses and most importantly allows our code to be *cleaner and scalable*. 
